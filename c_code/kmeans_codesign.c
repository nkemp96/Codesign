...
// Borrowed from Histo.c
   ...
   int num_vals, num_dims, num_clusters; 
...
// Read the data from the input file
   ...
   num_vals = Read2DData(MAX_STRING_LEN, MAX_DATA_VALS, infile_name, points_short, actual_clusters);

   if ( ComputeActualCentroids(num_vals, MAX_DATA_VALS, num_dims, points_short, actual_clusters) != num_clusters )
      { printf("ERROR: Number of clusters extracted from data file DOES NOT equal number specified on command line!\n"); exit(EXIT_FAILURE); }

   if ((points = (double *)malloc(sizeof(double) * num_vals * num_dims)) == NULL )
   ...
   if ((final_cluster_assignment  = (int *)malloc(sizeof(int) * num_vals)) == NULL )
      { printf("ERROR: Failed to allocate data 'final_cluster_assignment' array!\n"); exit(EXIT_FAILURE); }

// Convert the short data to double
   for ( point_num = 0; point_num < num_vals; point_num++ )
      for ( dim_num = 0; dim_num < num_dims; dim_num++ )
...
// Compute the clusters using the k-means algorithm.
   KMeans(num_dims, points, num_vals, num_clusters, centroids, final_cluster_assignment);
   gettimeofday(&t1, 0);
...
// Controller expects data to be transferred to the BRAM as the first operation.
   load_unload = 0;
   LoadUnloadBRAM(MAX_STRING_LEN, MAX_DATA_VALS, num_vals, load_unload, points_short, CtrlRegA, DataRegA, ctrl_mask);
